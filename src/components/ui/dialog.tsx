  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          "fixed top-1/2 left-1/2 z-50 flex max-h-[calc(100dvh-2rem)] w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 flex-col gap-4 overflow-y-auto rounded-2xl border border-border/80 bg-popover p-5 text-sm text-popover-foreground shadow-2xl ring-1 ring-foreground/5 duration-150 outline-none sm:max-w-lg sm:p-6 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
          className
        )}
        {...props}
      >
...
function DialogFooter({
  ...
  return (
    <div
      ...
      className={cn(
        "flex flex-col-reverse gap-2 pt-3 sm:flex-row sm:items-center sm:justify-end sm:gap-2.5",
        className
      )}
      {...props}
    >
